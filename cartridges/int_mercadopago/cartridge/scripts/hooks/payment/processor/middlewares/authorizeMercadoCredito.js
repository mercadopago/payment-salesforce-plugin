...
function setPaymentValid(
  ...
  Transaction.wrap(() => {
    ...
    paymentInstrument.custom.mercadoCreditoLink = paymentResponse.init_point;
...
function authorizeMercadoCredito(
  orderNumber,
  paymentInstrument,
  paymentProcessor
) {
  try {
    ...
    const paymentResponse = MercadopagoHelpers.preference.createRedirectingToCredits(paymentData);
    ...
    log.error("Error on authorizeMercadoCredito: " + e.message);
    return errorHandler();
  }
...
module.exports = authorizeMercadoCredito;
