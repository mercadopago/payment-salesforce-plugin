...
async function fillShippingForm(page) {
  ...
  if( !(await addNew.isVisible()) ) {
    ...
    await page.getByRole("textbox", { name: "* Address 1" }).fill(guestUser.address.street);
    await page.getByRole("textbox", { name: "Address 2" }).fill(guestUser.address.neighborhood);
